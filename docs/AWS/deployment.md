...
**目的**: 全体がパブリックにならないよう、CloudFrontにBasic認証をかけます。本プロジェクトのデプロイスクリプトは、環境変数を指定することで自動的にBasic認証の設定（KeyValueStoreの作成、認証関数のデプロイとアタッチ）を行います。

1. **環境変数の準備**
   以下の環境変数を設定してデプロイスクリプトを実行します。
